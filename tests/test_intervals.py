...
class Test(unittest.TestCase):
    ...
    def test_itv_cmp(self):
        a = (2, 4, False, True)

        # b is outside a on the left
        b = (0, 1, False, True)
        rel = intervals.compare(b, a)
        self.assertEqual(rel, intervals.IntervalRelation.OUTSIDE_LEFT)

        # b is overlaps a from left
        b = (0, 3, False, True)
        rel = intervals.compare(b, a)
        self.assertEqual(rel, intervals.IntervalRelation.OVERLAP_LEFT)

        # b is covered by a
        b = (2.5, 3.5, False, True)
        rel = intervals.compare(b, a)
        self.assertEqual(rel, intervals.IntervalRelation.COVERED)

        # b is equal to a
        b = (2, 4, False, True)
        rel = intervals.compare(b, a)
        self.assertEqual(rel, intervals.IntervalRelation.EQUALS)

        # b covers a
        b = (1.5, 4.5, False, True)
        rel = intervals.compare(b, a)
        self.assertEqual(rel, intervals.IntervalRelation.COVERS)

        # b is overlaps a from right
        b = (3, 6, False, True)
        rel = intervals.compare(b, a)
        self.assertEqual(rel, intervals.IntervalRelation.OVERLAP_RIGHT)

        # b is outside a on the right
        b = (5, 7, False, True)
        rel = intervals.compare(b, a)
        self.assertEqual(rel, intervals.IntervalRelation.OUTSIDE_RIGHT)
    ...
    def test_move(self):
        ...
        self.assertEqual(exit, [[0, 500, True, False]])
        self.assertEqual(enter, [[1000, 1500, True, False]])
        self.assertEqual(keep, [[500, 1000, True, False]])
    ...
    def test_overlap(self):
        ...
        self.assertEqual(left, [1, 4, True, False])
        self.assertEqual(right, None)
        ...
        self.assertEqual(op, "trunc")
        self.assertEqual(left, None)
        self.assertEqual(right, [10, 12, True, False])
        ...
        self.assertEqual(op, "trunc")
        self.assertEqual(left, [1, 4, True, False])
        self.assertEqual(right, [10, 10, True, True])
